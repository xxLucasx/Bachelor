...
Zum Beispiel beschreibt die Quelle \cite{Cal17} (S.208) das Problem, wie die United States of America am besten mit ihren Verbündeten kooperieren sollten.
...
Dabei ergibt sich die Frage, was bevorugt verwendt werden sollte.
...
\newline
Eine sichere Berechtigungsstruktur ist daher nötig, um diese Anforderung von der \ac{DSGVO} zu befriedigen.
Diese ist nämlich verletzt, wenn Daten, für die das Unternehmen verantwortlich ist, von ihrer Vertraulichkeit, Verfügbarkeit oder Integrität verletzt werden.
Deswegen sind Mitarbeiter, die mehr Berechtigungen besitzen, als sie benötigen, eine Risikostelle. \cite{EuroW}
\newline
...
Denn das Unternehmen muss sich im Klaren sein, wie viele Ressourcen das neue Konzept kostet, sowie der Punkt der Performance, da eine bessere Performance immer gut.
\begin{itemize}
...
\end{itemize}
Um diese quantifizieren zu können, wird eine Prioritätsanalyse (\ref{fig:Prio}) verwendet, um festzustellen, wie die Priorität für die Konzepte sein muss. \cite{BdIufH}
...
Die Berechtigung, die verloren gehen würden, würden eigene Profile bekommen und würden über ein anderes Sammelprofil dem Hauptprofil hinzugefügt werden.
...
Dies ist jedoch Zeitaufwendig und es besteht ein hohes Potenzial, dass Fehler durch nachlässiges Arbeiten geschehen.
\newline
Die zweite Möglichkeit wäre eine Algorithmus zu schreiben, welcher dies automatisiert.
Da weder die Zeit besteht noch die entsprechende Umgebung zum Testen eines Prototypen, kann die Entwicklung des Algorithmus innerhalb dieser Arbeit nur mittels Pseudocode stattfinden.
...
Die individuellen Sammelprofile werden basierend auf den Lese-/Schreibeprofilen generiert.
